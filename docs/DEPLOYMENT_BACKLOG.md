...
| `cf053b9` | `feat(missions): automatic daily checklist reset with mission_logs historical persistence` | Added `mission_logs` architecture for recurring daily protocol completions. The checklist automatically resets to unchecked on a new calendar day (whether missions were completed or uncompleted yesterday), while permanently retaining historical completions, cumulative XP, active days, and streak telemetry. | Verified & Passing (200+ tests) | **Skipped** (*Credit usage exceeded*) | **YES** (Included in latest batch) |
| `314d6a4` | `fix(prod): resolve Render PostgreSQL missing mission_logs table and subquery syntax errors` | Added `MissionLog` ORM model, Alembic migration, Postgres DDL self-healing, aliases for all derived subqueries, cross-DB safe mission toggle, and resilient `Promise.allSettled` in Goals.jsx. | Verified & Passing | **Backend Deployed to Render** (Frontend in backlog) | Backend Deployed to Render |
| `4b62162` | `fix(users): resolve Postgres date union type mismatch in streak calculation` | Fixed date type casting in Postgres union query for user streaks. | Verified & Passing | **Backend Deployed to Render** | Backend Deployed to Render |
| `bd2a04e` | `fix(frontend): add fetch timeout resilience, auth fallback, and authentic MKC brand favicon` | Added AbortController timeout to apiFetch (4s auth / 8s general), eliminated hanging duplicate fetch on GET errors, added 3.5s safety timer in AuthContext, added emergency login escape button in AppContent loading screen, and replaced Vite purple favicon with authentic MKC Anchor & Sword brand logo. | Verified & Passing (6/6 tests, npm build OK) | **Skipped** (*Credit usage exceeded*) | **YES** (Included in latest batch) |
| `ffef31f` | `fix(auth): resolve Render backend timeout, add DB connect_timeout, safe Redis fallback, and versioned MKC favicon` | Disabled localhost:6379 Redis connection attempts in production, added connect_timeout=10 to psycopg2 and SQLAlchemy, increased Gunicorn worker timeout to 120s, set login client timeout to 60s for cold-start tolerance, and implemented versioned favicon cache-busting (/mkc-favicon.svg?v=2). | Verified & Passing (6/6 tests, 17/17 auth tests) | **Pending Deployment** | Backend to Render, Frontend in backlog |
