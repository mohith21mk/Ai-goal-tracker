...
def setup_test_user(prefix: str, role: str = "user"):
    ...
    conn = get_connection()
    c = conn.cursor()
    c.execute("DELETE FROM password_resets WHERE user_id IN (SELECT id FROM users WHERE email LIKE ?)", (f"{prefix}%",))
    c.execute("DELETE FROM email_verifications WHERE user_id IN (SELECT id FROM users WHERE email LIKE ?)", (f"{prefix}%",))
...
def cleanup_test_user(uid: int):
    conn = get_connection()
    c = conn.cursor()
    c.execute("DELETE FROM password_resets WHERE user_id = ?", (uid,))
    c.execute("DELETE FROM email_verifications WHERE user_id = ?", (uid,))
...
def cleanup_by_email(email: str):
    conn = get_connection()
    c = conn.cursor()
    c.execute("DELETE FROM password_resets WHERE user_id IN (SELECT id FROM users WHERE email = ?)", (email,))
    c.execute("DELETE FROM email_verifications WHERE user_id IN (SELECT id FROM users WHERE email = ?)", (email,))
    c.execute("DELETE FROM user_settings WHERE user_id IN (SELECT id FROM users WHERE email = ?)", (email,))
