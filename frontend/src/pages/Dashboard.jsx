...
import { getMissions, toggleMission, getProgress, getDailyProgress, getTelemetry, getGoals, getDailyReflection, getProgression } from '../services/api';
...
const Dashboard = () => {
  ...
  const loadBackendData = useCallback(async () => {
    try {
      const [missionsData, telemetryRes, dailyRes, goalsRes, reflectionRes, progRes] = await Promise.all([
  ...
  const handleToggleMission = async (id) => {
    ...
    if (backendConnected) {
      try {
        ...
        const [freshTelemetry, freshDaily, freshProgression] = await Promise.all([
          getTelemetry().catch(() => null),
          getDailyProgress().catch(() => null),
          getProgression().catch(() => null),
        ]);
        ...
        if (freshDaily) setDailyProgress(freshDaily);
  ...
  return (
    <div className="dashboard-content-layout">
      ...
      <main className="center-content">
        ...
        {/* Section B: Overall Performance Metrics Section (4-Column Grid) */}
        <section className="metrics-section" id="analytics">
          <div className="section-header">
            <h2 className="section-title font-display">Performance Metrics</h2>
            <span className="section-subtitle">Overall Journey Telemetry</span>
          </div>

          <div className="metrics-grid-4col">
            {/* Row 1: Core Performance Pillars */}
            <StatCard
              title="Discipline Score"
              value={String(telemetry.discipline_score || 0)}
              unit="/100"
              subtitle="Normalized lifetime discipline"
            ...
            <StatCard
              ...
              unit="/100"
              subtitle="Mental fortitude & reflection"
            ...
            <StatCard
              ...
              unit="Days"
              subtitle={`Longest Record: ${telemetry.longest_streak || telemetry.streak_days || 0}d`}
        ...
        {/* Section A: Today's Daily Missions Protocol */}
        <section className="missions-section">
          <div className="section-header">
            <h2 className="section-title font-display">Daily Missions</h2>
            <span className="section-subtitle">
              {dailyProgress.missions_completed ?? completedCount}/{dailyProgress.total_missions ?? missions.length} Protocol Completed Today {loading && '(Syncing...)'}
            </span>
          </div>
      ...
      {/* 3. RIGHT INFORMATION PANEL (~320px Fixed Width) — Section A: Today's Progress HUD */}
      <aside className="right-panel">
        {/* 1. Today's Progress Circle */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '24px' }}>
          <ProgressCircle
            percentage={dailyProgress.completion_percentage}
            label={dailyProgress.completion_percentage > 0 ? "On Track" : "Starting"}
            disciplineScore={telemetry.discipline_score || 0}
            mindsetScore={telemetry.mindset_strength || 0}
            executionScore={dailyProgress.completion_percentage}
